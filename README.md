<!-- Typing Animation Header -->
<h1 align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=28&pause=1000&color=00C0FF&center=true&vCenter=true&width=600&lines=Hi+I'm+Praveen+Kumar!;Front-End+Developer+💻;Python+Learner+🐍;From+Tamil+Nadu,+India+🇮🇳" alt="Typing SVG" />
  </a>
</h1>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=praveen00735&label=Profile%20Views&color=00C0FF&style=for-the-badge" alt="profile views" />
</p>

---

### 👨‍💻 About Me
- 🔭 I’m currently working on *building responsive web pages and simple web apps*
- 🌱 I’m learning *JavaScript and Python for full-stack development*
- 💬 Ask me about *HTML, CSS, JavaScript, and Python*
- 📫 How to reach me: *praveensenthil539@gmail.com*


---

### 🧰 Languages and Tools
<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,python,github,vscode" />
</p>

---

### 🚀 My Current Goals
- ✅ Improve my *front-end design* and responsiveness skills  
- ✅ Build *mini full-stack projects* using JavaScript and Python  
- ✅ Contribute to *open-source* projects  
- ✅ Learn about *APIs, Databases, and Hosting*

---

### 🎯 My Future Goals
- 🚀 Become a *Full-Stack Developer* using React and Node.js  
- 🌍 Build a *personal portfolio website* hosted online  
- 💼 Get an *internship or freelance opportunity* in web development  
- 🤝 Collaborate with other developers on *open-source projects*  
- 📱 Create my own *web app* and publish it publicly  

---

### 💼 Projects I'm Working On
| Project Name | Description | Tech Stack |
|---------------|-------------|-------------|
| 🌐 *Portfolio Website* | My personal website to showcase projects and skills | HTML, CSS, JS |
| 📱 *Weather App* | Simple weather forecast app using API | HTML, CSS, JS |
| 🧮 *Calculator App* | A clean and simple calculator web app | HTML, CSS, JS |
| 🐍 *Python Mini Tools* | Basic Python scripts (like converters, games) | Python |

---

### 🏆 Achievements & Highlights
- 🥇 Completed multiple front-end practice projects  
- 🧠 Built a mini Python projects  
- 💡 Continuously improving coding logic and design skills  

---

### 📚 What I Use to Learn
- 🎓 *FreeCodeCamp* & *W3Schools* for daily practice  
- 📘 *MDN Web Docs* for references  
- 🧠 *ChatGPT* for debugging and quick concept learning  

---

### 💪 Tech Progress (My Skills)
| Skill | Progress |
|--------|-----------|
| HTML | ██████ 60% |
| CSS | ██████ 60% |
| JavaScript | ██████ 60% |
| Python | ██████ 60% |
| Git & GitHub | █████ 50% |

---

### 🎮 Hobbies & Interests
- 🕹 Gaming and exploring tech gadgets  
- 🎧 Listening to music while coding
- ⚽ i love to watch football matches
- 🌏 Learning new tech every day  

---

### 🤝 Support & Collaboration
💬 Open to collaborating on *beginner-friendly web projects*  
📬 Feel free to message me on *LinkedIn or Instagram* if you want to build something together!  

---

### 🌐 Connect with Me
<p align="center">
  <a href="https://www.linkedin.com/in/praveen-kumar-053389346?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>
  <a href="mail to:praveensenthil539@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" />
  </a>
  <a href="https://www.instagram.com/praveenkumar_spk_2k04?igsh=bXVoczM3OHVveWR0" target="_blank">
    <img src="https://skillicons.dev/icons?i=instagram" />
  </a>
</p>

---

⭐ *Thanks for visiting my profile!*  
Keep learning, keep building, and stay curious 🚀
